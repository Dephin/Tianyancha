...
def test(scrawl_times, req_times):
	db = Mysql(**db_conf)
	cookies = web_conf['cookies']

	for i in range(0, scrawl_times):
		scrawler = Scrawler.build_scrawler(cookies)
		scrawler.get_current_ip()
		if Scrawler.scrawler_num >= len(cookies):
		...
		for i in range(0, req_times):
			if req_num >= 10:
				...
				if Scrawler.scrawler_num > (len(cookies) - 1):
					Scrawler.scrawler_num = 0
				scrawler.set_cookie(cookies[Scrawler.scrawler_num])
				req_num = 0
